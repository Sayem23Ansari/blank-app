...
def get_blip_answer(image, question):
    ...
    answers = [blip_processor.decode(ans, skip_special_tokens=True) for ans in output.sequences]
    scores = output.sequences_scores.detach().cpu().numpy()
    ...
    if uploaded_file and question:
        if model_choice in ["BLIP", "Both"]:
            with st.spinner("Getting answer from BLIP..."):
                blip_answers, blip_scores = get_blip_answer(image, question)
                best_blip_answer = blip_answers[blip_scores.argmax()]
                st.success(f"🔵 **BLIP Answer:** {best_blip_answer}")

                # Plot BLIP beam search scores
                st.write("### BLIP Top-5 Generated Answers (Beam Search Scores)")
                fig, ax = plt.subplots(figsize=(6, 4))
                ax.barh(blip_answers[::-1], blip_scores[::-1], color="skyblue")
                ax.set_xlabel("Beam Score")
                ax.set_title("BLIP Prediction Confidence")
                st.pyplot(fig)
